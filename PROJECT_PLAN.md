...
A fully local, CPU-only agentic assistant that answers natural-language questions about your Gmail (and Calendar) history — e.g. *"Summarize all mails I sent to xyz in the past 3 months"* or *"List all interviews I was invited to this year"* — without sending any data to the cloud.
...
- Support for providers other than Gmail + Google Calendar (may extend later).
...
| Embedding model | **sentence-transformers** (`bge-small-en-v1.5` or similar) | Free, local, no Ollama dependency, small footprint |
| Vector store | **FAISS** | Lightweight, no server process, well understood |
| Metadata store | **SQLite** | Stores email/thread metadata (sender, recipient, date, label, thread_id) alongside FAISS index; joined at query time |
...
> Note: FAISS has no built-in metadata filtering, so all filtering (date range, sender, label) happens in SQLite first — FAISS is only queried for the semantic/content-similarity step, on the subset of message IDs SQLite already narrowed down.
...
- **First boot**: full backfill — paginate all messages via Gmail API, extract metadata into SQLite, embed subject+snippet into FAISS.
...
- Calendar sync follows the same incremental pattern via the Calendar API's `syncToken`.
...
├── models/                            # gitignored — GGUF + embedding model downloaded on first run
└── README.md
```
...
```python
def search_emails(
    query: str | None = None,       # free-text match against subject/body
    ...
    limit: int = 50,
) -> list[dict]:  # metadata rows only — no full body unless get_email_thread is called

def get_email_thread(thread_id: str) -> dict:  # full thread content, called selectively

def list_calendar_invites(
    date_from: str,
    date_to: str,
    status: str = "accepted",  # "accepted" | "all"
) -> list[dict]
```
...
1. **Data access layer** — Gmail/Calendar OAuth, SQLite schema, FAISS index, sync logic (Section 5)
...
3. **Model download on first run**: on first launch, auto-download the GGUF model file (and embedding model) from Hugging Face into `~/.local-email-agent/models/` with a progress indicator — same pattern as `ollama run` pulling a model on first use.
...
- Prioritize deterministic components: SQLite queries, Gmail/Calendar tools, classification, filtering, deduplication, aggregation, and sync logic.
- Mock external Gmail/Calendar API calls in unit tests; do not require a real Google account for the normal test suite.
